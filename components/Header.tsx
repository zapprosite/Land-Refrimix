    ...
    <header className="sticky top-0 z-50 bg-light-bg/80 dark:bg-dark-bg/80 backdrop-blur-sm shadow-md">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          <a href="#beneficios" className="hover:text-brand-gold transition-colors">
            Benefícios
          </a>
          <a href="#portfolio" className="hover:text-brand-gold transition-colors">
            Portfólio
          </a>
          ...
          {session ? (
            <button
              onClick={signOut}
              className="bg-brand-gold text-brand-black font-bold py-2 px-4 rounded-md hover:opacity-90 transition-opacity"
            >
              Sair
            </button>
          ) : (
        ...
        <div className="flex items-center">
          {session && <span className="hidden sm:inline mr-4 text-sm">{session.user.email}</span>}
